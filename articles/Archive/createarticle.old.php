...
EOF;
  ...
  $S->query($query);
  $S->query("insert into articlepreview values('$articleName', '$articleHeading', '$articleBody')");
...
function preview($S) {
  list($result, $n) = $S->query("select * from articlepreview", true);
  if($n != 1) {
    throw(new Exception("select articlepreview: n=$n"));
  }
  $row = $S->fetchrow($result);
  extract($row);
...
function make($S) {
  ...
  if(!$id) {
    list($nextid) = $S->fetchrow($S->query("select auto_increment from INFORMATION_SCHEMA.tables where table_name='articles'"));
  ...
  if(!empty($rssfeed)) {
    if(!$id) {
      ...
      // An Edit
      list($rssfeedId_fk) = $S->fetchrow($S->query("select rssfeedId_fk from articles where id='$id'"));

      echo "id=$id, rssfeedId_fk=$rssfeedId_fk<br>";
      
      $rsslink = "http://www.granbyrotary.org/articles/article.$id";

      list($articleHeading) = $S->fetchrow($S->query("select articleHeading from articlepreview"));
  ...
  $rssfeedId_fk = empty($rssfeed) ? 0 : ($t = $S->getInsertId()) ? $t : $rssfeedId_fk;
...
EOF;
  ...
  list($results, $n) = $S->query("select (expired < now()) as old, id, name, expired from articles", true);
...
EOF;
  ...
  if($id) {
    ...
    $results = $S->query("select name, rssfeedId_fk, articleInclude, rssfeed, article, expired from articles where id='$id'");
    $row = $S->fetchrow($results);
    extract($row);
