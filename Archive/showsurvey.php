...
function start($S) {
  $h->title = "Rotary Survery Summary";
  $h->banner = "<h1>Granby Rotary Club Survery Summary</h1>";
...
EOF;
  ...
  list($result, $n) = $S->query("select * from survey", true);
...
EOF;
      ...
      foreach($v1 as $k2=>$v2) {
        ...
        $option = $options[$k2];
        $tbl .= <<<EOF
<tr class="optrow"><th>$option</th><td>$v2</td></tr>
...
EOF;
  ...
  //vardump($comments, "comments");

  echo <<<EOF
$top
<h2>Total number of members responding to this survey: $n</h2>
$tbl
